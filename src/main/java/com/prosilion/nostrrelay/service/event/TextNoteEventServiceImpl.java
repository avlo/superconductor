...
public class TextNoteEventServiceImpl<T extends EventMessage> extends EventServiceImpl<T> {
  ...
  public TextNoteEventServiceImpl(T eventMessage) {
    super(eventMessage);
    repository = ApplicationContextProvider.getApplicationContext().getBean(EventRepository.class);
  }
  ...
  public IEvent processIncoming() {
    ...
    TextNoteEventDto textNoteEventDto = new TextNoteEventDto(sender.getPublicKey(), tags, "CONTENT");
    textNoteEventDto.setId("ID");

    TextNoteEventEntity t;
    try {
      t = textNoteEventDto.convertDtoToEntity();
