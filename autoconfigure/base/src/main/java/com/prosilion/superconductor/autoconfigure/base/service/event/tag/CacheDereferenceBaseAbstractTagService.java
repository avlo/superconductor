...
public abstract class CacheDereferenceBaseAbstractTagService<T extends ReferencedAbstractEventTag> {
  ...
  protected Optional<GenericEventRecord> sendConsolidatorReq(String relayUrl, Filters apply) {
...
//    List<BaseMessage> eventList = awaitUsingCompletableFuture(reqMessage, relayUrl);
    ...
    Optional<GenericEventRecord> first = getGenericEvents(eventList).stream().findFirst();
