...
public class JpaCacheService implements JpaCacheServiceIF {
  ...
  @Override
  public Optional<GenericEventRecord> getFirstEventByKindAndAddressTag(@NonNull Kind kind, @NonNull AddressTag addressTag) {
    return eventJpaEntityService.getFirstEventByKindAndAddressTag(kind, addressTag).stream()
       .filter(filterDeletionEvents())
       .findFirst()
       .map(EventIF::asGenericEventRecord);
  }
  ...
  public List<GenericEventRecord> getEventsByKindAndPubKeyTagAndEventTag(Kind kind, PubKeyTag referencePubKeyTag, EventTag eventTag) {
    return eventJpaEntityService.getEventsByKindAndPubKeyTagAndEventTag(kind, referencePubKeyTag, eventTag).stream()
       .filter(filterDeletionEvents())
       .map(EventIF::asGenericEventRecord).toList();
  }
